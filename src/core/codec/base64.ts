...
/**
 * Byte ⇄ base64 conversion for the AES envelope every `/api/v1` request is
 * wrapped in. (The voice assistant's PCM frames use `@live-assistant/core`'s
 * own copy, because a library package cannot import the app.)
 *
 * @remarks
 ...
 * - **Chunked, not per-byte** — building the binary string one `+=` at a
 *   time re-allocated it on each byte. `fromCharCode.apply` over a slice does the same work in one call;
 *   the slice size is bounded because a long argument list overflows the stack.
 */
// Small enough that the argument list stays well inside every engine's limit.

...
describe('AdsService', () => {
  ...
  it('stops asking after three attempts that reach no answer', async () => {
    ...
    for (let i = 0; i < 10; i += 1) await service.prepare();
  ...
  it('reports each failing step once per session', async () => {
    ...
    expect(crashed).toHaveBeenCalledWith(expect.any(Error), 'AdsService.gatherConsent');
  });

  it('falls back to the consent already stored on the device', async () => {
    mockGatherConsent.mockRejectedValue(new Error('offline'));
    mockGetConsentInfo.mockResolvedValue({ canRequestAds: true, status: 'OBTAINED' });
  ...
  it('reports the reason the SDK could not start', async () => {
    ...
    expect(crashed).toHaveBeenCalledWith(expect.any(Error), 'AdsService.initialize');
  });
});

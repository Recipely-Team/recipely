import {
  FEED_ROWS_BEFORE_FIRST_AD,
  FEED_ROWS_BETWEEN_ADS,
} from "@infrastructure/constants/ads";
...
const recipes = (count: number): readonly RecipeSummaryEntity[] =>
  Array.from(
    { length: count },
    (_, i) => ({ id: `r${i}` }) as RecipeSummaryEntity,
  );
...
const adPositions = (rows: ReturnType<typeof buildFeedRows>): number[] =>
  rows.reduce<number[]>(
    (acc, row, i) => (row.kind === FeedRowKind.Ad ? [...acc, i] : acc),
    [],
  );
...
describe("buildFeedRows — with ads on", () => {
  it("shows no ad in a feed too short to reach the first gap", () => {
    expect(
      adCount(buildFeedRows(recipes(FEED_ROWS_BEFORE_FIRST_AD), true)),
    ).toBe(0);
  });
  ...
  it("leaves the wider gap between one ad and the next", () => {
    const rows = buildFeedRows(
      recipes(FEED_ROWS_BEFORE_FIRST_AD + FEED_ROWS_BETWEEN_ADS + 2),
      true,
    );
  ...
  it("keeps every recipe, in order, however many ads land between them", () => {
    ...
    const kept = rows.flatMap((row) =>
      row.kind === FeedRowKind.Recipe ? [row.recipe.id] : [],
    );
  ...
  it("gives each ad its own ordinal", () => {
    ...
    const ordinals = rows.flatMap((row) =>
      row.kind === FeedRowKind.Ad ? [row.ordinal] : [],
    );

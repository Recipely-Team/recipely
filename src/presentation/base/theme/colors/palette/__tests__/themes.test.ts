...
const CONTRAST_PAIRS: ContrastPair[] = [
  ...
  {
    label: "primaryText vs primary",
    fg: "primaryText",
    bg: "primary",
    minRatio: 4.5,
  },
  {
    label: "secondaryText vs secondary",
    fg: "secondaryText",
    bg: "secondary",
    minRatio: 4.5,
  },
  ...
  {
    label: "textMuted vs background",
    fg: "textMuted",
    bg: "background",
    minRatio: 3.0,
  },
  {
    label: "onSuccess vs success",
    fg: "onSuccess",
    bg: "success",
    minRatio: 4.5,
  },
...
describe("themes — backgrounds are distinct (regression)", () => {
  it.each(VARIANTS)(
    "%s themes use a variety of background colors",
    (variant) => {
      const backgrounds = ALL_THEMES.map(
        (id) => getThemeColors(id, variant).background,
      );
  ...
  it("dark themes do not all share #0B0B0D", () => {
    const darkBackgrounds = ALL_THEMES.map(
      (id) => getThemeColors(id, "dark").background,
    );
    const sharedSlate = darkBackgrounds.filter(
      (bg) => bg.toUpperCase() === "#0B0B0D",
    );
  ...
  it("light themes do not all share #FFFFFF", () => {
    const lightBackgrounds = ALL_THEMES.map(
      (id) => getThemeColors(id, "light").background,
    );
    const sharedWhite = lightBackgrounds.filter(
      (bg) => bg.toUpperCase() === "#FFFFFF",
    );
...
describe.each(themeVariantCases)(
  "themes — onOverlay over photo backdrop — $id $variant",
  ({ id, variant }) => {
    const colors = getThemeColors(id, variant);
...
describe("themes — palette-wide distinctness (regression)", () => {
  ...
  for (const variant of VARIANTS) {
    for (let i = 0; i < ALL_THEMES.length; i++) {
      for (let j = i + 1; j < ALL_THEMES.length; j++) {

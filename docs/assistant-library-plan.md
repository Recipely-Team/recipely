...
- [ ] **Resume here (2026-09-11, stopped for the usage limit):**
  - Behaviour-equivalence audit (code-reviewer, old store vs new) was running when the session stopped — re-run it: `git diff 7b254624 -- src/application/assistant packages` against the old store at `git show 7b254624:src/application/assistant/session/assistant-session-store.ts`; nothing goes to dev before it says SAFE
  - Found live, PRE-EXISTING (not from the migration — registry and handlers are untouched; same code on dev/prod): `search` navigates and returns at once, so the registry reads the screen before results load and the model is told `recipes=none` → it says "bulamadım" while the list shows the recipe. Same class likely behind "tavuk şavurma → Klasik Çikolatalı Kurabiye" and "şakşuka → Fıstıklı Baklava" (openRecipe resolves against the list loaded at call time) and "yapılışını okuyamadı". Fix in `use-assistant-global-actions.ts`: wait for the feed to settle on the query (bounded) before returning; then re-verify openRecipe/readStep with a temporary `__DEV__` log of `{args, result}` in `assistant-live-tool.ts`

...
| Yayınlama onayı açıkken "çık" demek, altına ikinci bir sayfa açıyordu | Oluşturma ekranının `goBack`'i koşulsuz kayıtlıydı ve `onClose` başka sayfa açık mı diye bakmıyordu. Yayınlama onayı ekranda dururken "çık" denince çıkış sayfası **onun altında** açılıyor, `exitOrErrorOpen` true olduğu için yayınlama onayının evet/hayır'ı iptal ediliyor ve `cancel` artık taslağı **silmeye** bağlanıyordu — kullanıcı "Yayınlansın mı?" sayfasına bakarken sesli bir "hayır" işini siliyordu. | Ayrılmak da diğer onaylar gibi karşılıklı dışlamaya bağlı: başka bir sayfa açıkken `goBack` teklif edilmiyor, çıkış sayfası da onun arkasından cevaplanmıyor. **Kural: ekranda duran bir soru, yenisi sorulmadan önce cevaplanır.** |

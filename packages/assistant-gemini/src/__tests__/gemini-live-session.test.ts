...
describe('GeminiLiveSession', () => {
  const credentials: GeminiLiveCredentials = {
    token: 't',
  ...
  // forever — and a caller that opens the microphone first held the recording
  // device open with nothing counting against it.
  ...
  // which reads as "connecting, and then nothing". Every test here
  ...
  describe('authentication', () => {
    ...
    // An ephemeral token opens only the constrained method, so a consumer who
    // leaves the URL out must still land there rather than on one that refuses it.
    it('defaults to the constrained endpoint when no URL is given', async () => {
      const openedWith: string[] = [];
      const session = new GeminiLiveSession((url) => {
        openedWith.push(url);
        const socket = new FakeSocket();
      ...
      await session.connect({ token: 't', model: 'models/any' });

      expect(openedWith[0]).toMatch(/GenerativeService\.BidiGenerateContentConstrained\?access_token=t$/);
    });
  });
  ...
  it('answers a tool call under the name the model called it by', async () => {
    const { session, socket } = await connected();
    socket.sent.length = 0;

    session.respondToTool({ id: 'call-9', name: 'setAlarm' }, { ok: true, ctx: 'alarm=07:30' });
    const [frame] = socket.parsedSends() as unknown as {
    ...
    expect(frame!.toolResponse?.functionResponses?.[0]).toEqual({
      id: 'call-9',
      name: 'setAlarm',
      response: { ok: true, ctx: 'alarm=07:30' },
    });
  });
  ...
  it('forwards every event a frame carries, in the mapper\'s order', async () => {
    ...
    expect(events.map((e) => e.kind)).toEqual([SessionEventKind.Interrupted, SessionEventKind.Audio]);
  });
  ...
  it('reports a deliberate close as expected, even during a reconnect', async () => {
    ...
    expect(events).toEqual([{ kind: SessionEventKind.Closed, expected: true }]);
    session.close();
  });
  ...
  it('reports a dropped connection as unexpected', async () => {
    ...
    expect(events).toEqual([{ kind: SessionEventKind.Closed, expected: false }]);
  });
  ...
  it('survives a frame it cannot parse', async () => {
    ...
    expect(events).toEqual([{ kind: SessionEventKind.Usage, totalTokens: 12 }]);
  });
  ...
  it('keeps delivering to other listeners when one throws', async () => {
    ...
    expect(seen).toEqual([SessionEventKind.Usage]);
  });
  ...
  describe('binary frames', () => {
    ...
    it('reads a frame delivered as a typed-array view', async () => {
      ...
      expect(events).toEqual([{ kind: SessionEventKind.Usage, totalTokens: 5 }]);
    });
    ...
    it('still reads a text frame, so a proxy that reframes them keeps working', async () => {
      ...
      expect(events).toEqual([{ kind: SessionEventKind.Usage, totalTokens: 7 }]);
    });
  });
